    ...
    countEvents(0)
{
  ...
  event_sub_ = nh_.subscribe("events", 1, &Renderer::eventsCallback, this, ros::TransportHints().unreliable().reliable().tcpNoDelay(true)); //ros::TransportHints().unreliable().reliable().tcpNoDelay(true)
...
void Renderer::eventsCallback(const dvs_msgs::EventArray::ConstPtr& msg)
{
    ...
    countEvents += msg->events.size();
    ++countMsg;

    if (countMsg == 60)
    {
        std::cout << "Amount of events in array: " << countEvents << std::endl;
        countMsg = 0;
        countEvents = 0;
    }
